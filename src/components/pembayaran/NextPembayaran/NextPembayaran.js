...
export default function NextPembayaran(props) {
    ...
    const nav = [ {name: `ATM ${data.jenisBank}`, status: true},{name: `M-${data.jenisBank}`, status: false},{name: `${data.jenisBank} klik`, status: false},{name: `Internet Banking`, status: false} ]
    ...
    useEffect(() => {
        ...
        setInterval(() => {
            ...
            // pengurangan
            let secondsDifference = parseInt
            (secondsDeadline) - parseInt(secondsNow)
        ...
        <div className="nextPembayaran">
            <div className={"startHero " + (konfirmasi.judulButton !== "Konfirmasi Pembayaran" && " noneHp ")}>
                <div className="timeBayar">
                    {dateTime && <div className="containerJudul">
                        <h3 className="judul">Selesaikan Pembayaran Sebelum</h3>
                        <p className="textTime">{dateTime.namaHari},  {dateTime.tanggal} {dateTime.namaBulan} {dateTime.tahun}  jam {dateTime.jam}.{dateTime.menit} WIB</p>
                    </div>
                    }
                    {lessTime && <p className="time">
                        <span className="child">{lessTime.hour < 10 && "0"}{lessTime.hour}</span> : <span className="child">{lessTime.minutes < 10 && "0"}{lessTime.minutes}</span> : 
                        <span className="child">{lessTime.seconds < 10 && "0"}{lessTime.seconds}</span>
                    </p>
                     }
                ...
                <div className="nextTransfer">
                    ...
                    <div className="inputUser">
                        <label htmlFor=
                        "total">Total Bayar</label>
                        <p id="total" className="total" >Rp {total.toLocaleString("ID-id")}
                ...
                <div className="instruksiPembayaran">
                    ...
                    <div className="navsInstruksi">
                        {navs.map((res, index) => {
                            return (
                                <button onClick={() => handleNavigasi(index)} key={index} className={(res.status ? " instruksiActive " : " ") + "tombolNavs"}>{res.name}</button>
                            )
                        })}
                    </div>
                    ...
                    <div className="instruksi">
                        ...
                        <li>Masukkan nomor BCA Virtual Account: 70020+Order ID
                        Contoh:
            ...
            <div className="endHero">
                ...
                <div className="konfirmasiPembayaran">
                    ...
                    {konfirmasi.text && 
                        <p className="judulPembayaran">{konfirmasi.text} <br /> <br />
                        {konfirmasi.text2}
                        </p>
                    }
                    {konfirmasi.judul === "Konfirmasi Pembayaran" && 
                        <div className="gambarPreview">
                            {image ? <img src={image} alt="gambar bukti pembayaran" /> : <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-card-image" viewBox="0 0 16 16">
                    ...
                    <div className="containerKonfirmasi">
                        ...
                        {konfirmasi.judulButton === "Upload" ? <button              className="tombolKonfirmasi">
                        ...
                        <button className="tombolKonfirmasi tombolPesanan">
